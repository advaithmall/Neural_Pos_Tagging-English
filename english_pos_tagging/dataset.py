...
class PosDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.tag_to_index = {tag: index for index, tag in enumerate(self.uniq_tags)}
        self.index_to_tag = {index: tag for index, tag in enumerate(self.uniq_tags)}
    ...
    def load_words(self, args):
        def replace_with_unk(lst):
            # calculate the number of items to replace (1% of list length)
            num_items_to_replace = len(lst) // 100
            # randomly select the indices to replace
            idxs_to_replace = random.sample(range(len(lst)), num_items_to_replace)
            for idx in idxs_to_replace:
                lst[idx] = "<unk>"  # replace the item at the selected index with "<unk>"
            return lst
        file_name = "en_atis-ud-" + self.args.dataset + ".conllu"
    ...
    def get_uniq_words(self):
        words_all = dict()
        file_name = "en_atis-ud-" + "train" + ".conllu"
        file_1 = open(file_name)
        data = file_1.read()
        parsed = parse(data)
        for i in range(0, len(parsed)):
            for j in parsed[i]:
                words_all[j['form']] = 1
        # file_name = "en_atis-ud-" + "dev" + ".conllu"
        # file_1 = open(file_name)
        # data = file_1.read()
        # parsed = parse(data)
        # for i in range(0, len(parsed)):
        #     for j in parsed[i]:
        #         words_all[j['form']] = 1
        # file_name = "en_atis-ud-" + "test" + ".conllu"
        # file_1 = open(file_name)
        # data = file_1.read()
        # parsed = parse(data)
        # for i in range(0, len(parsed)):
