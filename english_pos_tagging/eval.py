...
def train(dataset, model, args):
    ...
    print("avg acc: ", sum(accuracy_list)/len(accuracy_list),
          "avg f1: ", sum(f1_list)/len(f1_list))
...
def eval(args, model, val_dataset):
    ...
    for batch, (sentence, val_tags) in enumerate(val_dataloader):
            ...
            pred_list = pred_list
            recall = recall_score(val_tags, pred_list,
                                    average='macro', zero_division=0)
            precision = precision_score(
                val_tags, pred_list, average='macro', zero_division=0)
            f1 = f1_score(val_tags, pred_list,
                            average='macro', zero_division=0)
            precision_list.append(precision)
            recall_list.append(recall)
            f1_list.append(f1)
            print("validation: ", {'batch': batch, 'acc': accuary.item(
            ), 'recall': recall, 'precision': precision, 'f1': f1})
    print("avg acc: ", sum(avg_list)/len(avg_list), "avg recall: ", sum(recall_list)/len(recall_list),
        "avg precision: ", sum(precision_list)/len(precision_list), "avg f1: ", sum(f1_list)/len(f1_list))
